...
- 对应里程碑：第二阶段完成边界固化
- 修改模块：`docs/specs/08-milestones-data-test.md` / `docs/records/MILESTONES.md` / `docs/records/CURRENT_STATUS.md` / `docs/records/HANDOFF.md`
- 修改内容：将第二阶段完成边界从隐含口径固化为显式规则：第二阶段主线以“节点与状态流转能力稳定、合同与回归测试收敛”为完成标准；Ubuntu 实机、前端联动、真实 `VLM/LLM/SmolVLA/机械臂` 链路与数据支线真实结果列为并行外部项，不再把它们混入第二阶段主线完成判断
- 对应需求点：阶段推进必须有明确的完成边界，既避免过早宣布端到端完成，也避免让外部验证项阻塞主线实现判断
- 风险和遗留问题：当前 records 里仍保留多组阶段性测试数字（13 / 17 / 21 / 22 / 24），后续对外汇总时仍需说明“阶段演进回归数”和“当前全量通过数”的差异
- 下一步建议：继续按该边界推进第二阶段主线代码实现，待主线能力收敛后再统一判断第二阶段是否完成
- 对应论文章节：系统实现与验证、测试设计与结果分析、工程化与交接规范
